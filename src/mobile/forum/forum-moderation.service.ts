...
export class ForumModerationService {
  ...
  private runRuleBasedChecks(text: string) {
    ...
    if (trimmedText.length < 3) {
      throw new BadRequestException('Nội dung bị từ chối do quá ngắn.');
    }
    ...
    if (repetitiveCharRegex.test(trimmedText)) {
      throw new BadRequestException('Nội dung bị từ chối do Spam.');
    }
    ...
    if (hasBlockedDomain) {
      throw new BadRequestException('Nội dung bị từ chối do Quảng cáo.');
    }
    ...
    if (cccdRegex.test(trimmedText)) {
      throw new BadRequestException('Nội dung bị từ chối do Tiết lộ thông tin cá nhân.');
    }
    ...
    if (sqlInjectionRegex.test(trimmedText) || htmlXssRegex.test(trimmedText) || jsUriRegex.test(trimmedText)) {
      throw new BadRequestException('Nội dung bị từ chối do Tấn công hệ thống.');
    }
    ...
    if (hasHatePattern) {
      throw new BadRequestException('Nội dung bị từ chối do Ngôn từ độc hại.');
    }
  ...
  private async runAiModeration(text: string) {
    try {
      ...
      if (result.isViolation) {
        const categoryName = result.category || 'Nội dung không phù hợp';
        throw new BadRequestException(`Nội dung bị từ chối do ${categoryName}.`);
      }
    } catch (error) {
